...
static void* apply_occupy(uint8_t base_idx, uint8_t total_req_cells)
{
    if (total_req_cells == 1)
    {
        ...
        void* allocated_ptr = heap_free_base_addr + PER_CELL_SIZE;
    ...
    void* allocated_ptr = heap_free_base_addr + (total_req_cells * PER_CELL_SIZE);
    heap_free_base_addr += total_req_cells;
    return allocated_ptr;
}
